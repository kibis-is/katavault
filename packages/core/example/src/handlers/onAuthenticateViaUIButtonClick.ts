...
// handlers
import onOpenVaultButtonClick from './onOpenVaultButtonClick';
  ...
  return async () => {
    ...
    try {
      ...
      if (activateButtonElement) {
        activateButtonElement.onclick = onOpenVaultButtonClick(katavault, logger);
        activateButtonElement.replaceChildren('Open Vault');
      }
    } catch (error) {
