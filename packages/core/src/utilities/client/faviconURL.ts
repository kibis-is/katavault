 ...
 */
export default function faviconURL(): string | null {
  const links: HTMLCollectionOf<HTMLElementTagNameMap['link']> = document.getElementsByTagName('link');
  ...
  for (const link of Array.from(links)) {
    ...
    if (href.indexOf('/') === -1) {
      href = window.location.pathname
        .split('/')
        .map((value, index, array) => (!href || index < array.length - 1 ? value : href)) // replace the current path with the href
        .join('/');
    }
