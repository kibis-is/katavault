 ...
 * @param {LogLevel} level - The base level of logging.
 * @returns {Logger} A logger that can be used to create logs based on the level.
 */
export default function createLogger(level: LogLevel = 'error'): Logger {
  const canLog: (allowedLevel: LogLevel) => boolean = (allowedLevel): boolean => {
    switch (level) {
      case 'error':
